...
- NP-RID-Sender-GH
- ESP32-C3-Board-PinMini

| 硬件外观                                                     | 硬件名称               | 说明                                                         |
| ------------------------------------------------------------ | ---------------------- | ------------------------------------------------------------ |
| ![image-20260805155713315](imgs\NP-RID-Sender-GH板子.png)    | NP-RID-Sender-GH       | 主控芯片：ESP32-S3<br />具备所有功能，可直接安装至无人机<br />连接外置天线<br />调试串口为UART2 |
| ![image-20260805160816575](imgs\image-20260805160816575.png) | ESP32-C3-Board-PinMini | 主控芯片：ESP32-C3<br />无外置天线、只支持蓝牙不支持WiFi广播，一般用于测试使用。<br />调试串口为UART0 |
...
这里以NP-RID-Sender-GH与雷迅CUAV V7+飞控为例进行连接说明。

#### 通过串口连接

可以产品附带的飞控串口连接线（4pin转6pin），连接设备UART1串口至飞控TELEM2串口。接线说明如下：

| 飞控TELEM2 | RID设备UART1（J6） |
| ---------- | ------------------ |
| 5V         | 5V                 |
| GND        | GND                |
| TX         | RX                 |
...
| 板子             | 当前版本 | 点击下载                                        |
| ---------------- | -------- | ----------------------------------------------- |
| NP-RID-Sender-GH | V1.0     | [OTA升级app固件](NP-RID-Sender-GH-V1.0_OTA.bin) |
|                  |          |                                                 |
